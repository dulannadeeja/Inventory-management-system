...
namespace InventoryManagmentApp
{
    public partial class Cashier : Form
    {
        ...
        {
            LblTitleBarUserName.Text = "Employee - " + UserName;

            SqlConnection ObjCon = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=|DataDirectory|\Database.mdf;Integrated Security=True;Connect Timeout=30");
            string Qry = "SELECT EMPID FROM Users WHERE UserName='" + UserName + "'";
            SqlCommand ObjCom = new SqlCommand(Qry, ObjCon);
            ObjCon.Open();
            SqlDataReader ObjDataReader = ObjCom.ExecuteReader();
            ObjDataReader.Read();
            string EMPID = ObjDataReader["EMPID"].ToString();
            TextIssuerID.Text = EMPID;
